...
class IrisEnv:
    ...
    def step(self, action):
        if self.index >= len(self.X) - 1:
            self.index = 0
        reward = self.reward(action[0], self.y[self.index])
        self.index += 1
        done = self.index >= len(self.y)
        next_state = self.X[min(self.index, len(self.y) - 1)]
        #print('a', action, 'y', self.y[self.index], 's', next_state, 'r', reward, 'd', done, 'corr')
    ...
    def reward(self, action, Y):
        ...
        normalized_distance = torch.clamp(1 - distance, 0, 1)  # Normalize to [0, 1]
        reward = normalized_distance.item()
        return reward
...
class PPO:
    ...
    def train(self, states, actions, old_log_probs, rewards, dones):
        ...
        for _ in range(self.epochs):
            ...
            #print(actions)
            clamped_probs = torch.clamp(new_action_probs, min=1e-10)
